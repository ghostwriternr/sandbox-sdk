...
After extensive analysis, the core problem is straightforward:

1. **Platform operations need secrets** (e.g., deploying to your AWS account)
2. **User operations may need different secrets** (e.g., deploying to user's AWS account)  
3. **Generated code should run without any platform secrets**

The key insight: **You always know which credentials each operation needs** because you're explicitly calling it.

### The Clean Solution

```typescript
// Platform operations - you control these directly
await sandbox.exec('aws s3 cp s3://my-templates/starter.zip .', {
  env: { 
    AWS_ACCESS_KEY_ID: myPlatformKey,
    AWS_SECRET_ACCESS_KEY: myPlatformSecret
  }
});

// User operations - if user has AWS
await sandbox.exec('claude -p "Deploy to RDS"', {
  env: {
...
// Generated code - no secrets
await sandbox.exec('node app.js');
```

### Implementation Approach

When credentials are passed to `exec()`, we automatically isolate them using Linux namespaces:

```typescript
// In bun server
async function executeHandler(req) {
  const { command, env } = await req.json();
  
  if (env && Object.keys(env).length > 0) {
    // Has credentials? Run in isolated namespace
    return await executeInNamespace(command, env);
  } else {
    // No credentials? Run normally
    return await executeNormally(command);
  }
}

async function executeInNamespace(command: string, env: Record<string, string>) {
  // Use unshare to create isolated namespace
  const proc = spawn(['unshare', 
    ...
    env: { ...minimalEnv, ...env }, // Secrets only exist here
    cwd: '/app'
  });
  
  // Secrets are isolated to this namespace only
